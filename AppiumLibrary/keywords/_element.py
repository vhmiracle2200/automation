...
class _ElementKeywords(KeywordGroup):
    ...
    def find_element_by_xpath(self, xpath):
        ...
        """
        if self.element_find("xpath=" + xpath, False, False):
            self._info("Clicking element '%s'." % xpath)
            element =self.element_find("xpath=" + xpath, False, False)
            return element
    ...
    def click_element(self, locator):
        ...
        self.element_find(locator, True, True).click()
    ...
    def click_text(self, text, exact_match=False):
        ...
        """
        self.element_find_by_text(text,exact_match).click()
    ...
    def element_should_be_disabled(self, locator, loglevel='INFO'):
        ...
        `introduction` for details about locating elements.
        """
        if self.element_find(locator, True, True).is_enabled():
            self.log_source(loglevel)
    ...
    def element_should_be_enabled(self, locator, loglevel='INFO'):
        ...
        `introduction` for details about locating elements.
        """
        if not self.element_find(locator, True, True).is_enabled():
            self.log_source(loglevel)
    ...
    def element_should_be_visible(self, locator, loglevel='INFO'):
        ...
        """
        if not self.element_find(locator, True, True).is_displayed():
            self.log_source(loglevel)
    ...
    def element_attribute_should_match(self, locator, attr_name, match_pattern, regexp=False):
        ...
        """
        elements = self.element_find(locator, False, True)
        if len(elements) > 1:
    ...
    def get_webelement(self, locator):
        ...
        New in AppiumLibrary 1.4.
        """
        return self.element_find(locator, True, True)
    ...
    def get_webelements(self, locator):
        ...
        New in AppiumLibrary 1.4.
        """
        return self.element_find(locator, False, True)
    ...
    def get_matching_xpath_count(self, xpath):
        ...
        New in AppiumLibrary 1.4.
        """
        count = len(self.element_find("xpath=" + xpath, False, False))
        return str(count)
    ...
    def text_should_be_visible(self, text, exact_match=False, loglevel='INFO'):
        ...
        New in AppiumLibrary 1.4.5
        """
        if not self.element_find_by_text(text, exact_match).is_displayed():
            self.log_source(loglevel)
    ...
    def xpath_should_match_x_times(self, xpath, count, error=None, loglevel='INFO'):
        ...
        New in AppiumLibrary 1.4.
        """
        actual_xpath_count = len(self.element_find("xpath=" + xpath, False, False))
    ...
    def element_find(self, locator, first_only, required, tag=None):
        application = self._current_application()
        elements = None
        if isstr(locator):
            _locator = locator
            elements = self.element_finder.find(application, _locator, tag)
    ...
    def element_find_by_text(self, text, exact_match=False):
        if self._get_platform() == 'ios':
            element = self.element_find(text, True, False)
            if element:
                return element
            else:
                if exact_match:
                ...
                return self.element_find(_xpath, True, True)
